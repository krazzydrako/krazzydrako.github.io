...
# the course.
#
# Artifact Description: This thermostat project focuses on a software application 
# that manages temperature settings in a home environment. 
# Main features include basic temperature regulation and user interface elements for adjustments.
#
# Justification for Inclusion: This project exemplifies skills in software design, 
# showcasing the ability to create functional applications that meet user needs. 
# Enhancements include remote temperature monitoring and user notifications for temperature thresholds.
#
# Reflection on the Enhancement Process: Enhancing the thermostat project was challenging yet enlightening, 
# particularly in ensuring the remote monitoring feature was secure and user-friendly.
#
# Functionality:
#
...
##
## Configure server URL for remote monitoring
##
REMOTE_SERVER_URL = "http://your-remote-server.com/temperature"  # Replace with your server URL
...
class TemperatureMachine(StateMachine):
    ...
    def on_enter_heat(self):
        self.updateLights()  # Update lights when entering heat state
        if DEBUG:
    ...
    def on_exit_heat(self):
        redLight.off()  # Turn off the red light when exiting heat
    ...
    def on_enter_cool(self):
        self.updateLights()  # Update lights when entering cool state
        if DEBUG:
    ...
    def on_enter_off(self):
        redLight.off()  # Turn off the red light when entering the off state
        blueLight.off()  # Turn off the blue light when entering the off state
        if DEBUG:
    ...
    def processTempStateButton(self):
        ...
        self.cycle()  # Change the state of the thermostat
    ...
    def processTempIncButton(self):
        ...
        self.setPoint += 1  # Increase set point by 1 degree
        self.updateLights()  # Update lights after changing set point
    ...
    def processTempDecButton(self):
        ...
        self.setPoint -= 1  # Decrease the set point by one degree
        self.updateLights()  # Update lights after changing set point
    ...
    def updateLights(self):
        ...
        if self.current_state == self.heat:
            if temp < self.setPoint:
                redLight.pulse()  # Fade in and out if heating and temp is below point
            else:
                redLight.on()  # Solid on if heating and temp is at or above set point
                
        elif self.current_state == self.cool:
            if temp > self.setPoint:
                blueLight.pulse()  # Fade in and out if cooling and temp is above set point
            else:  
                blueLight.on()  # Solid on if cooling and temp is at or below set point
    ...
    def setupSerialOutput(self):
        ...
        self.sendRemoteTemperatureData()
    ...
    ##
    def sendRemoteTemperatureData(self):
        try:

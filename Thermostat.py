...
class TemperatureMachine(StateMachine):
    ...
    def setupSerialOutput(self):
        ...
        ## Create the output string with the state, current Temperature, and set point
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            ## Setup display line 1 with the current date and time
            
            lcd_line_1 = current_time.strftime("%Y-%m-%d %H:%M:%S") ## Format YYYY-MM-DD HH:MM:SS
            ...
            if(altCounter < 6):
                ...
                lcd_line_2 = f"Temp: {floor(self.getFahrenheit())}F" ## Show current temperature
    
                altCounter = altCounter + 1
            else:
                
                lcd_line_2 = f"state: {self.current_state.id}, Set: {self.setPoint}F" ## Show state and set point
            ...
            if((counter % 30) == 0):
                ...
                ser.write(self.setupSerialOutput().encode()) ## send current state information over URAT
...
greenButton.when_pressed = tsm.processTempStateButton ## Change the thermostat state on button press
...
redButton.when_pressed = tsm.processTempIncButton ## increase set point on button press
...
blueButton.when_pressed = tsm.processTempDecButton ## Decrease set point on button press
